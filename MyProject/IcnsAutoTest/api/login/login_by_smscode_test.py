...
class LoginBySMSCodeTest(unittest.TestCase):
    ...
    def tearDown(self):
        # 使用类对象代替全局变量
        print("\t用例%02d的响应数据：%s" % (LoginBySMSCodeTest.clanum, self.result))
        LoginBySMSCodeTest.clanum += 1
...
if __name__ == '__main__':
    ...
    tests = []
    i = 1
    while i <= 13:  #跟测试案例最后一个数字一致，增加案例，这里就要改
        tests.append(LoginBySMSCodeTest("test_%d_login" %i))
        i += 1
