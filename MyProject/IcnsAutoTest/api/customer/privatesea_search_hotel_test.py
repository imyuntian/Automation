...
class PrivateSeaSearchHotel(unittest.TestCase):
    ''' 测试公海搜索酒店接口 '''

    def setUp(self):
        self.url = 'https://testcns.xiudream.com/api/customer/private_sea/search_hotel'
        self.headers = {'Content-Type': 'application/raw'}
        self.cookies = self.login()
    
    clanum = 1
    def tearDown(self):
        print("\t用例%02d的响应数据：%s" % (PrivateSeaSearchHotel.clanum, self.result))
        PrivateSeaSearchHotel.clanum += 1
...
if __name__ == "__main__":
    ...
    while i <= 1:  #跟测试案例最后一个数字一致，增加案例，这里就要改
        tests.append(PrivateSeaSearchHotel("test_%d_list" %i))
        i += 1
